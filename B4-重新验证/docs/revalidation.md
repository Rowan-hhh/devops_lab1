...
B4 接收 B2 已接受的修复上下文，并确认候选 Patch 在约定环境下真正生效。进入 B4 前必须具备：
...
B4 工具不应用 Patch，也不替 B2 判断报告是否完整。它验证的是已经准备好的 candidate workspace。
...
真实 REPAIR 请求仍须满足主契约中 `clean_command`、`build_command`、`test_command` 和 `configuration_id` 的要求；本请求只是 B4 执行记录的本地入口。
...
每个阶段的 stdout/stderr 写入 `b4-evidence/<stage>.log`。报告记录退出码、耗时、相对日志路径；未执行阶段记录 `skipped` 和原因。超时阶段记录 `timed_out`，其后的阶段不执行。
...
报告中出现未被本次 Patch 选择的其他 Finding，不会单独导致本次候选被拒绝，但 B4+B2 应在最终确认时记录是否需要另行处理。
...
```json
{
  ...
  "validation": {
    "build": {"success": true, "exit_code": 0, "duration_ms": 123, "log_path": "b4-evidence/build.log"},
    "test": {"success": true, "exit_code": 0, "duration_ms": 123, "log_path": "b4-evidence/test.log"},
    "recheck": {"success": true, "exit_code": 0, "duration_ms": 123, "log_path": "b4-evidence/recheck.log"}
  },
...
B4 的 `ACCEPTED` 只证明当前候选完成 rebuild、test、recheck 且选中 Finding 消失。B4 与 B2 仍需共同确认接受或拒绝 Patch；接受后由流程发起方决定如何提交和推送。
