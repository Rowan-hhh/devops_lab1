...
本目录对应流程七“重新验证”。B4 在 B2/MDFixer 产出并接受候选 Patch 后，使用同一构建环境和配置执行 rebuild、test、recheck，并把证据交给 B4+B2 做最终接受或拒绝决定。
...
- B4 不修改源代码、不执行 `git apply`、不创建 commit，也不自动推送或合并。
- B4 只在候选 workspace 中执行 clean（若配置）、build、test 和 recheck。
...
| [`examples/revalidation-request.example.json`](examples/revalidation-request.example.json) | B4 验证请求结构样例 |
...
| [`tools/b4_revalidate.py`](tools/b4_revalidate.py) | Python 标准库验证器和命令行入口 |
| [`tests/test_b4_revalidate.py`](tests/test_b4_revalidate.py) | 本地单元测试和失败路径测试 |
...
python B4-重新验证/tools/b4_revalidate.py \
  ...
  --output <candidate-workspace>/b4-verification.json
```
...
- B4 验证记录是对 REPAIR `validation` 的过程性补充，不替代 `RepairSuccessOutput`，不修改根目录 Schema。
